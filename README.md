...
- **Decision Logic**:
  - If front sector has >0.35m clearance: go forward
...
- **Two-Phase Control**:
  - Phase 1: Navigate to target position (x, y) while maintaining forward motion (0.2 m/s)
  - Phase 2: Rotate in place to achieve target orientation (theta)
- **Proportional Control**: Direct angular velocity proportional to angle error
...
- **Tolerances**: 0.1m position tolerance, 0.1 rad orientation tolerance (~5.7°)
...
### Simulation vs Real Robot Configuration

This main branch is configured for **Gazebo simulation**. The `real-robot` branch contains modifications for physical hardware deployment.

| Configuration | Main Branch (Simulation) | Real-Robot Branch |
|---------------|-------------------------|-------------------|
| **Topics** | `/scan`, `/cmd_vel`, `/odom` | `/fastbot_1/scan`, `/fastbot_1/cmd_vel`, `/fastbot_1/odom` |
| **Laser Range** | -90° to +90° (-π/2 to +π/2) | 0° to 360° (0 to 2π) |
| **Sector Angles** | Right: -90° to -30°<br>Front: -30° to +30°<br>Left: +30° to +90° | Right: 270° to 330°<br>Front: 330° to 30° (wraps)<br>Left: 30° to 90° |
| **Clearance Threshold** | 0.35 m | 0.5 m |
| **Go-To-Pose Linear Vel** | 0.2 m/s | 0.1 m/s |
| **Go-To-Pose Angular Control** | Direct proportional | Clamped (0.4 to 1.5 rad/s) with gain 0.8 |
| **Orientation Tolerance** | 0.1 rad (~5.7°) | 0.5 rad (~28.6°) |
| **Feedback Rate** | 1 Hz | 10 Hz |

To use on real hardware, switch to the `real-robot` branch:

```bash
git checkout real-robot
colcon build --packages-select robot_patrol
```
...
- **Sector division**: Three 60° sectors covering front 180°
  - Right: -90° to -30° (-π/2 to -π/6)
  - Front: -30° to +30° (-π/6 to +π/6)
  - Left: +30° to +90° (+π/6 to +π/2)
- **Front clearance threshold**: 0.35 m (35 cm)
- **Decision priority**: Front preferred if clear, otherwise largest distance sector

### Go-To-Pose Action Parameters

- **Linear velocity**: 0.2 m/s (during navigation phase)
- **Angular velocity**: Proportional to angle error (direct mapping)
- **Position tolerance**: 0.1 m (10 cm)
- **Orientation tolerance**: 0.1 rad (~5.7°)
- **Control frequency**: 10 Hz
- **Feedback rate**: 1 Hz (position updates in logs)
...
- **Real-time Feedback**: Continuous position updates during goal execution (1 Hz)
- **Cancelable**: Goals can be canceled mid-execution
- **Smooth Control**: Direct proportional control for natural motion in simulation
...
- **Simulation Configuration**: Main branch parameters optimized for Gazebo simulation
- **Branch Strategy**:
  - `main`: Simulation-ready with standard topic names and simulation-tuned parameters
  - `real-robot`: Hardware-ready with robot-specific topics and real-world tuning
